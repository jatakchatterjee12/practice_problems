...
//Approach-2 (Using set)
//T.C : O(n) ///Note : O(n) because while loop will run max 5 times(taking first small number = 2). So if constraint is given, then saying O(n) time complexity is fine.
//S.C : O(n)
class Solution {
    public int longestSquareStreak(int[] nums) {
