    ...
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        for(int row = 0; row<9; row++) {
            ...
            for(int col = 0; col <9; col++) {
                char ch = board[row][col];
                if(ch == '.') continue;
                if(st.count(ch)) return false; // count > 0
                st.insert(ch);
            }
        }
...
//Approach-2 (One iteration using hashset and indexing boxes)
//see the diagram (in the link above) to understand the indexing of the boxes
class Solution {
public:
    bool isValidSudoku(vector<vector<char>>& board) {
...
//see the diagram (in the link above) to understand how I have numbered the boxes
class Solution {
public:
    bool isValidSudoku(vector<vector<char>>& board) {
        bool row[9][9] = {0};
...
//************************************************** JAVA ******************************************************//
//Approach-1 Naive Approach same as above cpp code
    ...
    public boolean isValidSudoku(char[][] board) {
        ...
        for(int i=0; i<9; i++){
            for(int j=0; j<9; j++){
                ...
                String row = String.valueOf(board[i][j]) + "_ROW_" + Integer.toString(i);
                String col = String.valueOf(board[i][j]) + "_COL_" + Integer.toString(j);
                String box = String.valueOf(board[i][j]) + "_BOX_" + Integer.toString(i/3) + Integer.toString(j/3);
