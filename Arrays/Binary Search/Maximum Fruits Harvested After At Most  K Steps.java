...
class Solution {
    public int maxTotalFruits(int[][] fruits, int startPos, int k) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            prefixSum[i+1] = fruits[i][1] + prefixSum[i];
        }
        ...
        for(int d = 0; d <= k/2; d++){
            ...
            int right = upperBound(pos, j);

            int total = prefixSum[right] - prefixSum[left];
            ...
            right = upperBound(pos, j);
    
            total = prefixSum[right] - prefixSum[left];
            maxFruits = Math.max(total, maxFruits);  
        }  
