...
class Solution {
    ...
    void dfs(int[][] land, int i, int j, int[] bounds) {
        land[i][j] = 0;
        ...
        for(int i = 0; i < m; i++) {
            for(int j = 0; j < n; j++) {
                ...
                if(land[i][j] == 1) {
                    ...
                    bounds[0] = r1;
                    bounds[1] = c1;
                    dfs(land, i, j, bounds); // will fill the bounds[2] and bounds[3]
                    res.add(bounds);
                }
            }
        }
...
//Approach-1 (Using DFS) Another way to store the points
//T.C : O(m*n)
//S.C : O(1) AUxiliary space and O(m*n) system stack space because of recursion
class Solution {
    int m;
