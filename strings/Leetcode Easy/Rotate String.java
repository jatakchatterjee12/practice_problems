...
//Approach-2 (Same as Approach 1 with different masala)
//TC - O(N^2)
//SC - O(1)
class Solution {
    public boolean rotateString(String s, String goal) {
    ...
    boolean rotate(String s, String goal, int rotateCount) {
        for(int i = 0; i < s.length(); i++) {
            if(s.charAt(i) != goal.charAt((i+rotateCount)%goal.length())){
                return false;
            }
        }
