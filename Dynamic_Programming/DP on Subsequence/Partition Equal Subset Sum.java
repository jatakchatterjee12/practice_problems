...
// Another way
class Solution {
    private boolean solve(int[] nums, int i, int targetSum, int n, Boolean[][] dp){
        if(targetSum == 0) return true;
        if(i == n-1){
            return nums[i] == targetSum;
        }
        ...
        boolean not_take = solve(nums, i+1, targetSum, n, dp);
        boolean take = false;
        if(nums[i] <= targetSum){
            take = solve(nums, i+1, targetSum-nums[i], n, dp);

        }

        return dp[i][targetSum] =  not_take || take;
    }
