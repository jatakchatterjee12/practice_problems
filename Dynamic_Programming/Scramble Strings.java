    ...
    boolean solve(String s1, String s2) {
        ...
        for(int i = 1; i < n; i++) {
          ...
          /*
            Example : "great"  "eatgr"
            if swapped at i = 2
            we compare s1.substr(0, i) i.e "gr"  with s2.substr(n-i, n) i.e. "gr"
            && s1.substr(i, i+n-i) i.e "eat"  with s2.substr(0, n-i) i.e. "eat"
          */

            boolean swapped = solve(s1.substring(i, n), s2.substring(0, n-i)) && solve(s1.substring(0, i), s2.substring(n-i, n));
            if(swapped == true){
                result = true;
                break;
            }
          ...
          /*
            ...
            if not swapped at i = 2
            we compare s1.substr(0, i) i.e "gr"  with s2.substr(0, i) i.e. "gr"
            && s1.substr(i, i+n-i) i.e "eat"  with s2.substr(i, i+n-i) i.e. "eat"
          */

            boolean non_swapped = solve(s1.substring(0,i), s2.substring(0,i)) && solve(s1.substring(i,n), s2.substring(i, n));
            if(non_swapped == true){
                result = true;
                break;
            }
