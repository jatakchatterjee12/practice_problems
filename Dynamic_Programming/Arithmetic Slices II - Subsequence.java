/*
    codestorywithMIK YOUTUBE VIDEO ON THIS Qn : https://www.youtube.com/watch?v=cau0JE1q_ZA
...
public:
    int numberOfArithmeticSlices(vector<int>& nums) {
        int n = nums.size();
        int result = 0;
        unordered_map<long, int> mp[n]; // array of map that stores {diff , count}
...
public class Solution {
    public int numberOfArithmeticSlices(int[] nums) {
        int n = nums.length;
        int result = 0;
        Map<Long, Integer>[] mp = new HashMap[n]; // array of map that stores {diff , count}
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                int count_at_i = mp[i].getOrDefault(diff, 0);

                result += count_at_j;
                
                // Increment the count at index i
                mp[i].put(diff, count_at_i + count_at_j + 1);
            }
        }
        return result;
    }
