...
//Approach 3 (using Ordered_set or SortedSet)
//TC- O(nlogn)
//SC - O(n)
class Solution {
    public long findScore(int[] nums) {
        ...
        int n =  nums.length;
        SortedSet<int[]> st = new TreeSet<>((a,b)->a[0]==b[0]?a[1]-b[1]:a[0]-b[0]);
        ...
        while(st.size() > 0) {
            ...
            if(idx-1 >= 0){
                int[] find_left = new int[]{nums[idx-1], idx-1};
                if(st.contains(find_left)){
                    st.remove(find_left);
                }
            }

            if(idx+1 <= n-1){
                int[] find_right = new int[]{nums[idx+1], idx+1};
                if(st.contains(find_right)){
                    st.remove(find_right);
                }
            }
