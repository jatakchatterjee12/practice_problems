...
// Using Comparator  --> Easy to understand for me
...
class MovieRentingSystem {
    ...
    Map<Integer, TreeSet<Node>> available;      // movie -> {shop,movie,price}
    Map<Integer, Map<Integer, Node>> movieToShopPrice; // movie -> (shop -> {shop,movie, price})
    TreeSet<Node> rented;                     // sorted rented movies {shop, movie, price}
    ...
    public MovieRentingSystem(int n, int[][] entries) {
        ...
        for (int[] e : entries) {
            ...
            available.computeIfAbsent(movie, k -> new TreeSet<>(CMP)).add(node);
            movieToShopPrice.computeIfAbsent(movie, k -> new HashMap<>()).put(shop, node);
        }
    }
    ...
    public void rent(int shop, int movie) {
        Node node = movieToShopPrice.get(movie).get(shop);
