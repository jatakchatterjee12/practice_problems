...
/////////////////////////
class Solution {
    void update(int number, int[] vec, int val){
    ...
    public int minimumSubarrayLength(int[] nums, int k) {
        ...
        while(j < n){
            ...
            update(nums[j], vec, 1); // adding the OR of nums[j] to the commulative OR

            while(i <= j && getDecimal(vec) >= k){
                result = Math.min(result, j-i+1);
                update(nums[i], vec, -1); // deleting OR of nums[i] from the cummulative OR
                i++;
            }
            j++;
        }
